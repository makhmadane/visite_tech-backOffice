...
import { Ka_typeCommentaireService } from "src/app/services/Ka_typeCommentaire.service";
import { Ka_type_commentaireModel } from "src/app/models/ka_type_commentaireModel.model";



@Component({
  selector: "ka_type_commentaire",
  templateUrl: "./ka_type_commentaire.component.html",
  styleUrls: ["./ka_type_commentaire.component.css"],
})
export class ka_type_commentaireComponent implements OnInit {

  p: number = 1;
  tps: any[];

  id: any;
  tp: Ka_type_commentaireModel;
  allDossier=false;


  constructor(
    private tpService: Ka_typeCommentaireService,
  ...
  ngOnInit() {
    this.spinner.show();
    this.tp = new Ka_type_commentaireModel();
  ...
  delete(id) {
    ...
    this.tpService.deleteData(id).subscribe(
      ...
      Swal.fire({
        position: 'top-end',
        icon: 'success',
        title: data.message=="SUCCESS"?"Type commentaire supprimé avec succés":data.message,
        background:'white',
    ...
    if (this.id){
      this.tpService.updateData(this.id,this.tp).subscribe(
        ...
        container.click();
          ...
          position: 'top-end',
          icon: 'success',
          title: 'Type commentaire mise à jour avec succés',
          background:'white',
          showConfirmButton: false,
          timer: 3000
        });
      ...
      ()=>{
        ...
        Swal.fire({
          position: 'top-end',
          icon: 'success',
          title: 'Type commentaire a été ajouté avec succès',
          background:'white',
          showConfirmButton: false,
          timer: 3000
        });
